...
function AutoChen.OnUpdate()
  ...
  if Menu.IsKeyDown(AutoChen.ComboKey) and Entity.IsAlive(myHero) and not Entity.IsDormant(myHero) and Entity.GetHealth(myHero) > 0 then
    ...
    AutoChen.UseHeroAbilities(myHero, enemy, myMana, faith, penitence, faith_max_damage, penitence_amp, attackPoint)
  end
  if Menu.IsKeyDown(AutoChen.ComboKey) then
  ...
  if (AutoChen.IsSuitableToUseItem(myHero) or AutoChen.IsChannellingAbility(myHero)) then
    AutoChen.item_glimmer_cape(myHero)
  end
  if AutoChen.IsSuitableToUseItem(myHero) and NPC.IsVisible(myHero) then
    AutoChen.item_sheepstick(myHero)
    AutoChen.item_orchid(myHero)
    AutoChen.item_rod_of_atos(myHero)
    AutoChen.item_abyssal_blade(myHero)
    AutoChen.item_veil_of_discord(myHero)
    AutoChen.item_dagon(myHero)
    AutoChen.item_hand_of_midas(myHero)
    AutoChen.deward(myHero)
    AutoChen.item_lotus_orb(myHero)
    AutoChen.item_solar_crest(myHero)
  end
end
...
function AutoChen.UseHeroAbilities(myHero, enemy, myMana, faith, penitence, faith_max_damage, penitence_amp, attackPoint)
...
function AutoChen.item_hand_of_midas(myHero)
  ...
  for i, npc in ipairs(creeps) do
    ...
    if NPC.IsCreep(npc) and not AutoChen.IsAncientCreep(npc) and XP >= XP_threshold then
...
function AutoChen.deward(myHero)
...
function AutoChen.item_sheepstick(myHero)
  ...
  for i, enemy in ipairs(enemyAround) do
    if not NPC.IsIllusion(enemy) and not AutoChen.IsDisabled(enemy)
    and AutoChen.CanCastSpellOn(enemy) and not AutoChen.IsLotusProtected(enemy) then
...
-- Doesn't use on enemy who is lotus orb protected or AM with aghs.
function AutoChen.item_orchid(myHero)
  ...
  for i, enemy in ipairs(enemyAround) do
    if not NPC.IsIllusion(enemy) and not AutoChen.IsDisabled(enemy)
    and AutoChen.CanCastSpellOn(enemy) and not NPC.IsSilenced(enemy) and not AutoChen.IsLotusProtected(enemy) then
      local dis = (Entity.GetAbsOrigin(myHero) - Entity.GetAbsOrigin(enemy)):Length()
      if dis < minDistance then
        minDistance = dis
        target = enemy
...
-- Doesn't use on enemy who is lotus orb protected or AM with aghs.
function AutoChen.item_rod_of_atos(myHero)
  ...
  for i, enemy in ipairs(enemyAround) do
    if not NPC.IsIllusion(enemy) and not AutoChen.IsDisabled(enemy)
    and AutoChen.CanCastSpellOn(enemy) and not AutoChen.IsLotusProtected(enemy)
...
-- Doesn't use on enemy who is lotus orb protected or AM with aghs.
function AutoChen.item_abyssal_blade(myHero)
  ...
  local target = nil
  for i, enemy in ipairs(enemyAround) do
    if not NPC.IsIllusion(enemy) and not NPC.IsStunned(enemy) and not AutoChen.IsLotusProtected(enemy) then
      local dis = (Entity.GetAbsOrigin(myHero) - Entity.GetAbsOrigin(enemy)):Length()
      if dis < minDistance then
        minDistance = dis
        target = enemy
...
function AutoChen.item_dagon(myHero)
  local level, item
  ...
  for i, enemy in ipairs(enemyAround) do
    if not NPC.IsIllusion(enemy) and not AutoChen.IsDisabled(enemy)
    and AutoChen.CanCastSpellOn(enemy) and AutoChen.IsSafeToCast(myHero, enemy, magic_damage) then
...
function AutoChen.item_veil_of_discord(myHero)
  ...
  local radius = 600
  local pos = AutoChen.BestPosition(enemyHeroes, radius)
...
function AutoChen.item_lotus_orb(myHero)
  local item = NPC.GetItem(myHero, "item_lotus_orb", true)
  if not item or not Ability.IsCastable(item, NPC.GetMana(myHero)) then return end

  -- cast on self first if needed
  if AutoChen.NeedToBeSaved(myHero) then Ability.CastTarget(item, myHero); return end
  ...
  for i, ally in ipairs(allyAround) do
    if AutoChen.NeedToBeSaved(ally) and AutoChen.CanCastSpellOn(ally) then
  ...
  if not NPC.HasModifier(myHero, "modifier_item_lotus_orb_active") and AutoChen.CanCastSpellOn(myHero) then
    Ability.CastTarget(item, myHero)
    return
  end
  ...
  for i, ally in ipairs(allyAround) do
    if Entity.IsAlive(ally) and not NPC.IsIllusion(ally) and AutoChen.CanCastSpellOn(ally)
...
function AutoChen.item_solar_crest(myHero)
  local item
...
function AutoChen.item_glimmer_cape(myHero)
  local item = NPC.GetItem(myHero, "item_glimmer_cape", true)
  if not item or not Ability.IsCastable(item, NPC.GetMana(myHero)) then return end

  if AutoChen.CanCastSpellOn(myHero) and (AutoChen.NeedToBeSaved(myHero) or AutoChen.IsChannellingAbility(myHero)) then
    Ability.CastTarget(item, myHero)
  end
  ...
  for i, ally in ipairs(allyAround) do
    if AutoChen.CanCastSpellOn(ally) and (AutoChen.NeedToBeSaved(ally) or AutoChen.IsChannellingAbility(ally)) then
      Ability.CastTarget(item, ally)
      return
    end
  end
